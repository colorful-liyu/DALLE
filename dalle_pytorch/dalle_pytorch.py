...
class DALLE(nn.Module):
    ...
    def forward(
        ...
        text = torch.where(text == 0, text_range, text) # Where text == 0 is replaced with text_range, else text

        # add <bos>
        # mynotes: why text_pos_emb don't +1 ?

        text = F.pad(text, (1, 0), value = 0)
        # mynotes: text shape after padding is [b, 81]

        tokens = self.text_emb(text)

        # mynotes: self.text_pos_emb(torch.arange(text.shape[1])) shape is[81, 256]
        tokens += self.text_pos_emb(torch.arange(text.shape[1], device = device))

        # mynotes: tokens shape after pos_emb is [b, 81, 256]

        seq_len = tokens.shape[1]

        # mynotes: image.shape is torch.Size([b, 3, 256, 256])
        if exists(image) and not is_empty(image):
            ...
            # mynotes: image_len is 256
            # mynotes: image_emb.shape is torch.Size([b, 256, 256])

            # mynotes: here the pos_emb is not edcoded by the position but embedding
            image_emb += self.image_pos_emb(image_emb)

            tokens = torch.cat((tokens, image_emb), dim = 1)
            # mynotes: total tokens.shape is torch.Size([b, 337, 256])

            seq_len += image_len
            # mynotes: seq_len is 337
        ...
        # mynotes: total tokens.shape after minus is torch.Size([b, 336, 256])

        out = self.transformer(tokens)
        # mynotes: out shape is torch.Size([b, 336, 256])

        if self.stable:
            out = self.norm_by_max(out)

        logits = self.to_logits(out)
        # mynotes: logits is torch.Size([b, 336, 50512])

        # mask logits to make sure text predicts text (except last token), and image predicts image

        logits_mask = self.logits_mask[:, :seq_len]
        # mynotes: logits_mask.shape is torch.Size([1, 336, 50512])
        max_neg_value = -torch.finfo(logits.dtype).max
        logits.masked_fill_(logits_mask, max_neg_value)
        ...
        # mynotes: self.num_text_tokens is 49488
